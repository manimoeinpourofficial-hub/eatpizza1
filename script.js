...
let tc=0,lt=0;c.addEventListener("touchstart",()=>{if(!start){start=true;return;}if(go){reset();return;}
...
addEventListener("keydown",e=>{if(e.code==="Space"){if(!start)start=true;else if(go)reset();else{if(gun)shooting=true;else shoot();}}});
...
function reset(){reds=[];obs=[];greens=[];blues=[];bullets=[];buffs=[];score=ammo=0;prob=0.3;go=false;hunger=miss=0;gun=false;shooting=false;pizzaCount=0;}
...
function upd(){
 ...
 reds.forEach((r,i)=>{r.y+=2*sp;if(coll(p,r)){score+=5;if(score>hs){hs=score;localStorage.setItem("hs",hs);}pizzaCount++;if(pizzaCount%2===0)ammo++;reds.splice(i,1);}
 else if(r.y>H){reds.splice(i,1);miss++;if(miss>=3)go=true;}});
 obs.forEach((o,i)=>{o.y+=2.2*sp;if(coll(p,o)){go=true;}else if(o.y>H)obs.splice(i,1);});
 greens.forEach((g,i)=>{g.y+=1.8*sp;if(coll(p,g)){hunger=Math.max(0,hunger-15);prob=Math.max(0,prob*0.85);greens.splice(i,1);}});
 blues.forEach((b,i)=>{b.y+=1.6*sp;if(coll(p,b)){hunger=Math.min(100,hunger+10);prob=Math.min(1,prob*1.1);blues.splice(i,1);}});
 buffs.forEach((bf,i)=>{bf.y+=2*sp;if(coll(p,bf)){if(bf.t==="s"){speedBoostUntil=Date.now()+10000;if(miss>0)miss--;}if(bf.t==="g"){gun=true;gunUntil=Date.now()+10000;}buffs.splice(i,1);}else if(bf.y>H)buffs.splice(i,1);});
 bullets.forEach((b,i)=>{b.y-=b.s;if(b.y+b.h<0)bullets.splice(i,1);obs.forEach((o,j)=>{if(coll(b,o)){obs.splice(j,1);score+=3;bullets.splice(i,1);}})});
...
function draw(){
 ...
 if(go){x.fillStyle="rgba(0,0,0,0.6)";x.fillRect(0,0,W,H);x.fillStyle="#fff";x.textAlign="center";x.textBaseline="middle";x.font="40px Arial";
 x.fillText("Game Over",W/2,H/2-20);x.font="20px Arial";x.fillText("Tap/Space Restart",W/2,H/2+20);x.fillText("High:"+
 if(go){
